...
function globalAuthValidation(form) {
    ...
    const conf_password = $(form).find("input[name=password_confirmation]");
    const new_password = $(form).find("#new_password");
    const new_password_conf = $(form).find("#new_password_conf");
    const old_password = $(form).find("#old_password");

    if (name.length > 0) {
        if (!usernameReg.test(name.val())) {
            name.siblings(".errors").text("نام وارد شده معتبر نیست!");
            name.parent().siblings(".errors").text("نام وارد شده معتبر نیست!");
        } else {
            name.siblings(".errors").text("");
            name.parent().siblings(".errors").text("");
        }
    }
    ...
    if (password.length > 0) {
        if (!passwordReg.test(password.val())) {
            password.siblings(".errors").text("رمزعبور وارد شده معتبر نیست!");
            password
                .parent()
                .siblings(".errors")
            ...
            if (conf_password.length > 0) {
                if (password.val() !== conf_password.val()) {
                    ...
                    conf_password.parent().siblings(".errors").text("");
                }
            }
        ...
        if (!passwordReg.test(new_password.val())) {
            ...
            if (new_password_conf.length > 0) {
                if (new_password.val() !== new_password_conf.val()) {
                    ...
                    new_password_conf.parent().siblings(".errors").text("");
                }
            }
