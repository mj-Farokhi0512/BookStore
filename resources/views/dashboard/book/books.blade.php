...
@section('content')
    <div class="card">
        ...
        <div class="px-4 py-2">
            <a class="btn btn-primary text-white" href="{{ route('books.reportExcel') }}">Excel</a>
            <a class="btn btn-danger text-white" href="{{ route('books.reportPdf') }}">PDF</a>
        </div>
        <div class="d-flex px-4 align-items-center justify-content-between">
            <div class="d-flex align-items-center">
                <label>مرتب سازی :</label>
                <select id="sort_select" class="selectpicker ms-2" data-style="btn-default"
                    onChange="location = this.value;">
                    <option class="d-none"></option>
                    <option value="{{ route('books') }}/?sort_by=name"
                        {{ request()->query('sort_by') == 'name' ? 'selected' : '' }}>
                        براساس
                        نام
                    </option>
                    <option value="{{ route('books') }}/?sort_by=date"
                        {{ request()->query('sort_by') == 'date' ? 'selected' : '' }}>
                        براساس تاریخ
                    </option>
                </select>
            </div>
            <div class="d-flex align-items-center">
                ...
                <div class="input-group input-group-merge ms-2">
                    ...
                    <input type="text" class="form-control" placeholder="جستجو..." aria-label="Search..."
                        aria-describedby="basic-addon-search31" id="book_search" />
                </div>
            </div>
        </div>
